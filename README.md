...
| **Admin panel** | A `/admin` area behind the `admin` realm role: event CRUD, a one-shot seating-layout builder with a live seat/revenue preview, live event stats, a box-office orders console (find an order by its number or customer, see it whole, refund it), seat release, and a gate scanner that reads QR codes with the browser's own `BarcodeDetector`, which a steward holding only the `scanner` role can open without the rest of the panel. |
| **Admin API** | Events & layout management, live event stats, order search and full-order views, refunds and their retries, seat release — all `ROLE_ADMIN`. |
...
stats, the orders console and its refunds, seat release, and the gate scanner.
