...
| **Async notifications** | An independent service consumes booking events (idempotently, with a DLQ) and sends templated email via Mailpit/SMTP: booking confirmations, cancellations that say what happened, and a refund email sent only once the provider has accepted the refund. |
